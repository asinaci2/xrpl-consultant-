...
-   **XRPL Ecosystem Directory**: A curated, browsable reference of XRPL/blockchain projects organized into 8 categories (DeFi/AMM, RWA/Tokenization, Payments, NFTs & Gaming, Infrastructure, Sidechain/EVM, DAOs, Emerging). Each entry has name, description, website, X handle, token ticker, XRPL feature tags, and a status badge. Accessible via a view toggle on the Directory page (`"consultants" | "ecosystem"` state). Admin manages entries via the Ecosystem tab in the Admin dashboard. Constants (`ECOSYSTEM_CATEGORIES`, `ECOSYSTEM_STATUS_OPTIONS`, `ECOSYSTEM_STATUS_COLORS`) all live in `constants.ts`. Schema: `ecosystem_projects` table with `text("xrpl_features").array()` and `text("consultant_slugs").array()`. Admin can import from consultant projects via a "Source from Consultant Projects" panel that pre-fills the form. Each ecosystem card shows linked consultant avatar badges that link to their profile pages. Each ecosystem category section auto-discovers consultants via their `ecosystemAlignments` field and shows a "Consultants active in this space" strip.
-   **Consultant Services**: `consultant_services` table stores named service offerings per consultant (title, description, icon, display order, active flag). Dashboard ServicesTab (Content group) provides full CRUD. `GET /api/c/:slug/services` returns active services publicly. `Services.tsx` on consultant profiles renders real service entries first (icon + title + description cards) with a fallback to specialty-category cards if no entries exist.
-   **Ecosystem Engagement Layer**: Ecosystem alignment badges on consultant hero and Services section are clickable anchor links to `/?view=ecosystem&cat=...`. Directory page reads `?view=ecosystem` and `?cat=...` URL params on mount to support deep-linking from consultant profiles. Directory consultant view includes an "Ecosystem Focus" filter row (purple pills per ECOSYSTEM_CATEGORY) that filters consultants by their `ecosystemAlignments`. All filters are exclusive (ecosystem filter clears specialty/category filters and vice versa).
...
-   **Dashboard tabs**: `client/src/components/dashboard/` — each tab is its own file (`ProfileTab.tsx`, `ProjectsTab.tsx`, `ServicesTab.tsx`, `StoriesTab.tsx`, `MediaTab.tsx`, `ChatWidgetTab.tsx`, `TestimonialsTab.tsx`, `ContactInfoTab.tsx`, `WalletTab.tsx`, `VisitorContactsTab.tsx`, `VisitorTestimonialsTab.tsx`, `ExpertiseTab.tsx`). Shared context in `context.ts`, types in `types.ts`, constants in `constants.ts`.
